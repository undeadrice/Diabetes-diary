...
public class APIConnection {
	...
	/**
	 * connects to Nutritionix database and parses the received JSON. Returns fields
	 * of all the hits in form of a List
	 * 
	 * @param query
	 * @return
	 * @throws IOException
	...
	public ObservableList<MealEntry> getMealEntries(String query) throws IOException {
		Result result = getResult(query);
		List<MealEntry> list = new ArrayList<>();

		for (Product p : result.getHits()) {
			list.add(new MealEntry(p));
		}
		ObservableList<MealEntry> observableList = FXCollections.observableArrayList(list);
